...
import com.github.situx.postagger.dict.pos.cuneiform.HittitePOSTagger;
...
public class HittiteCorpusHandler extends CuneiCorpusHandler {
    ...
    @Override
    public String cleanWordString(String word, final boolean reformat,final boolean lowercase) {
    ...
    public DictHandling generateCorpusDictionaryFromFile(final List<String> filepath,final String signpath,final String filename,final boolean wholecorpus,final boolean corpusstr,final TestMethod testMethod) throws IOException, ParserConfigurationException, SAXException {
        ...
        dicthandler.setCharType(CharTypes.HITTITE);
            ...
            if(!notakk && !line.isEmpty() && line.substring(0,1).matches("[0-9]")){
                ...
                for(String word:words){
                    ...
                    if(word.contains("{") && word.contains("}") ){
                        ...
                        HittiteChar akkad;
                        if(chara==null){
                            akkad=new HittiteChar(this.cleanWordString(word.substring(word.indexOf("{")+1,word.indexOf("}")),true,true));
                        ...
                        dicthandler.addWord(akkad, CharTypes.HITTITE);
                        //lastchar=akkad;
    ...
    public POSTagger getPOSTagger(Boolean newPosTagger) {
        if(this.posTagger==null || newPosTagger){
            this.posTagger=new HittitePOSTagger();
        }
    ...
    public DictHandling getUtilDictHandler() {
        if(this.utilDictHandler==null){
            this.utilDictHandler=new HittiteDictHandler(new LinkedList<>());
            try {
                this.utilDictHandler.importMappingFromXML("dict/hit_map.xml");
                this.utilDictHandler.importDictFromXML("dict/hit_dict.xml");
                //this.utilDictHandler.importMappingFromXML("dict/hit_map.xml");
