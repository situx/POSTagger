 ...
 */
public class HittitePOSTagger extends CuneiPOSTagger {

    /**
     * Constructor for this class.
     */
    public HittitePOSTagger() {
        super(new TreeMap<>(), CharTypes.HITTITE);
    }
    ...
    public java.util.List<Integer> getPosTag(String word, final DictHandling handler) {
        ...
        if(result.isEmpty() || onlyVerbmatch && lastmatched!=null && lastmatched.getTag().equals("NN")){
            ...
            /*if((!matched && Translator.isAllUpperCaseOrNumberOrDelim(lastmatchedword)) || (matched && !onlyVerbmatch && Translator.isAllUpperCaseOrNumberOrDelim(lastmatchedword) && result.isEmpty())){
                result.clear();
                result.add(0, this.poscolors.get("namedentity").getRGB());
                result.add(0);
                result.add(word.length());
                POSDefinition posdef=new POSDefinition("","","","",new String[0],"namedentity","","","",new TreeMap<Integer,List<GroupDefinition>>());
                posdef.setPosTag(POSTags.NAMEDENTITY);
                posdef.setClassification("NE");
                posdef.currentword=word;
                posdef.setValue(new String[]{word});
                posdef.setEquals("");
                posdef.setRegex(generalPattern);
                lastmatched=posdef;
    ...
    public List<POSDefinition> getPosTagDefs(String word, final DictHandling handler/*,final Boolean dummy*/) {
        ...
        Boolean onlyVerbmatch=true,namedentity;
        POSDefinition tempmatch=unknownPOS,
                firstmatch=null;
        word = word.replace("]", "").replace("[", "");
        for (Integer key : this.classifiers.keySet()) {
        ...
        /*if(result.isEmpty() && lastmatched!=null && lastmatched.getTag().equals("NN") && Translator.isAllUpperCaseOrNumberOrDelim(lastmatchedword)){
            result.clear();
            POSDefinition posdef=new POSDefinition("","","","",new String[0],"namedentity","","","",new TreeMap<>());
            posdef.setPosTag(POSTags.NAMEDENTITY);
            posdef.setClassification("NE");
            posdef.currentword=word;
            posdef.setValue(new String[]{word});
                ...
                case VERB:  if(classificationResult.size()>position+1){
                    ...
                    switch (next){
                        case CONJUNCTION: if(!words[position+1].replace("[","").replace("]","").equals("u3")){
                            ...
                            collectwords.delete(0,collectwords.length());
                        }
                            break;
                        default:
                            for(int i=beginline;i<=currentline+1;i++){
